...
def iteration_suppression(arg):
    ...
    suppressed_database_number_of_elements = data.size
...
def generate_iterations_suppressed_database(output_file_name, df, path_average_distances, m_and_M, numberofrepeat: int=500):
...
def MoS_Laplace_and_Gaussian(output_file_name, base_file, m_and_M, value_range, epsilon, delta, EpsDeltaChange=True):
...
def M_Laplace_and_Gaussian(output_file_name, df, m_and_M, epsilon, delta, value_range, EpsDeltaChange=True, numberofrepeat: int=500):
    sumtotal = df.sum()
    original_average = df.mean()
