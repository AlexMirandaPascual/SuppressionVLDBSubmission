...
def generate_plot_uniform_Poisson_sampling(plot_path_start, csv_path_list_M, csv_path_list_MoSChange, epsilon_list, statistic, error_type, mechanism_name, include_title=True):
    fig, ax = plt.subplots()
    ...
    for eps, csv_path_M, csv_path_MoSChange in zip(epsilon_list, csv_path_list_M, csv_path_list_MoSChange):
        ...
        if error_type=="AE":
            ...
            term = 1/df_M_all["original_average"].iloc[0]
            ylabel_error = "relative error"
        elif error_type=="PE":
            term = 100/df_M_all["original_average"].iloc[0]
        ...
        df_M_plot = df_M.filter(["m","stat_AE_"+mechanism_name],axis=1).sort_values(by=["m"])
        df_MoSChange_plot = df_MoSChange.filter(["m","stat_AE_"+mechanism_name],axis=1).sort_values(by=["m"])
        ...
        color = next(colors)
        plt.plot(1-df_M_plot["m"],df_M_plot["stat_AE_"+mechanism_name]*term, color=color, linestyle = "solid", label = "$\\mathcal{M}$ satisfying "+DP_label)
        plt.plot(1-df_MoSChange_plot["m"],df_MoSChange_plot["stat_AE_"+mechanism_name]*term, color=color, linestyle = "dotted", label = "$\\mathcal{M}\\circ\\mathcal{S}$ satisfying "+DP_label)
