...
for eps in [0,0.25,0.5,0.75,1,2]:
    ...
    CS_areas = plt.contourf(p, q, z, countours_eps_suppression, colors=my_colors, alpha=0.4, extend="max")
    ...
    if eps!=0:
        CSblue_area = plt.contourf(p, q, z, [0,eps], colors=my_blue, alpha=0.2)
