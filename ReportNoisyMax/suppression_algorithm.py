...
def check_if_RNM_returns_real_maximum(counts, epsilon, delta, value_range, real_maximum_index):  
    ...
    value_count_maximum = counts[real_maximum_index]
...
"""Compute MoS for RNM"""
def MoS_RNM(output_file_name, df, path_average_distances, m_and_M, value_range, epsilon, delta, EpsDeltaChange=True, numberofrepeat: int=500):
...
def M_RNM(output_file_name, df, m_and_M, value_range, epsilon, delta, EpsDeltaChange=True, numberofrepeat: int=500):
    counts=df.value_counts()
    real_maximum_index = counts.index[0] ##Maximum will be the first item of counts
...
def DifferenceBetweenMetrics(output_file_name,path_MoS_stat, path_MoS_ChangeEpsDelta_stat, path_M_stat, path_M_ChangeEpsDelta_stat, m_and_M):
    ...
    for m, M in m_and_M:
        ...
        ## We compute the empirical probability of being incorrect, and thus add the -. This ensures that "larger values = larger utility". 
