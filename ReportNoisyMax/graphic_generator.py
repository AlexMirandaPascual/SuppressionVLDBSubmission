...
def generate_plot_uniform_Poisson_sampling(plot_path_start, csv_path_list_M, csv_path_list_MoSChange, epsilon_list, noise_name, include_title=True):
    fig, ax = plt.subplots()
    ...
    for eps, csv_path_M, csv_path_MoSChange in zip(epsilon_list, csv_path_list_M, csv_path_list_MoSChange):
        ...
        color = next(colors)
        plt.plot(1-df_M_plot["m"],1-df_M_plot["correct_empirical_probability_"+noise_name], color=color, linestyle = "solid", label = "$\\mathcal{M}$ satisfying "+DP_label)
        plt.plot(1-df_MoSChange_plot["m"],1-df_MoSChange_plot["correct_empirical_probability_"+noise_name], color=color, linestyle = "dotted", label = "$\\mathcal{M}\\circ\\mathcal{S}$ satisfying "+DP_label)
    ...
    plt.savefig(plot_path_start+".pdf", bbox_inches='tight')
    plt.close()